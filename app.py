...
# --- 【バグ修正・UI改善】アカウント切り替えと認証フローをサイドバーに独立 ---
with st.sidebar.expander("👤 カレンダーアカウント設定", expanded=False):
    st.caption("現在連携しているGoogleアカウントの接続を解除し、別のアカウントに切り替えます。")
    
    SCOPES = ['https://www.googleapis.com/auth/calendar']
    
    # すでにtoken.jsonが存在する場合（連携済み）
    ...
    # 未連携（token.jsonがない）の場合、ここで認証UIを構築する
    else:
        st.warning("⚠️ 現在アカウントは連携されていません")
        if os.path.exists('credentials.json'):
            # 【重要】画面が再描画されても消えないように、flow（暗号キー）をsession_stateに記憶させる
...
if st.button("📅 カレンダーに登録"):
    try:
        with st.spinner("Googleカレンダーに通信中..."):
            ...
            # --- 【バグ修正・UI改善】メイン側の登録ボタンから不要なUIフローを削除 ---
            # 認証情報がない場合は、左上のメニューから認証するようにユーザーを促して安全にストップさせます。
            if not creds or not creds.valid:
                if creds and creds.expired and creds.refresh_token: 
                    creds.refresh(Request())
                else: 
                    st.error("❌ 認証情報がありません。左上の「＞」メニューを開き、「👤 カレンダーアカウント設定」からログインを完了させてください。")
                    st.stop()
